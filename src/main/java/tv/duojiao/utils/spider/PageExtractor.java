...
//        logger.info("imgUrlList is: {}", imgUrlList.toString());
     ...
     * @return 图片地址
     */
    public static List<String> getImageList(String str, int maxSize) {
        Pattern p_image;
        Matcher m_image;
        List<String> imageList = new ArrayList<>();
        String regEx_img = "(http|https)://(\\w+\\.)+(\\S+)[\\S/.\\-]*(jpg|jpeg|gif|png)"; //图片链接地址
     ...
     * @return 更新后的html代码
     */
    public static String removeImgExtraTags(Elements imgHtml) {
        Pattern p_image;
        Matcher m_image;
        ...
        for (Element element : imgHtml) {
            ...
            while (m_image.find()) {
                stringBuffer.append("<p align=\"center\"><img src=\"" + m_image.group() + "\"></p>");
            }
        }
